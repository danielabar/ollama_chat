...
module Ollama
  class Client
    ...
    def request(prompt, cached_context, chat_id)
      request = Net::HTTP::Post.new(@uri, "Content-Type" => "application/json")
      request.body = {
        model: "mistral:latest",
        prompt: context(prompt),
      ...
      Net::HTTP.start(@uri.hostname, @uri.port) do |http|
        ...
        yield rand if block_given? # yield random value if block given
        http.request(request) do |response|
          response.read_body do |chunk|
            yield chunk if block_given? # yield chunk if block given
          end
        end
      end
    end
...
class ChatJob < ApplicationJob
  ...
  def perform(prompt, chat_id)
    ...
    client.request(prompt, cached_context, chat_id) do |rand, chunk|
      if rand # if rand is provided, broadcast initial empty message
...
```ruby
def process_chunk(chunk, rand, chat_id)
  process_json_chunk(chunk, chat_id)
end

private

def process_json_chunk(chunk, chat_id)
  json = JSON.parse(chunk.force_encoding("UTF-8"))
  done = json["done"]
  message = generate_message(json)

  log_done_message(chat_id) if done

  cache_context(chat_id, json["context"]) if done
end

def generate_message(json)
  json["response"].to_s.strip.empty? ? "<br/>" : json["response"]
end

def log_done_message(chat_id)
  Rails.logger.info("🎉 Done streaming response for chat_id #{chat_id}.")
end

def cache_context(chat_id, context)
  cache_key = "context_#{chat_id}"
  # cache context for next inference
end
```
